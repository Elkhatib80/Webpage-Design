...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <article className="group relative bg-surface rounded-2xl border border-white/5 overflow-hidden card-hover flex flex-col">
      {/* Badges */}
      ...
      <Link href={`/products/${product.slug}`} className="block">
        <div className="relative h-52 bg-gradient-to-br from-dark-2 to-dark-3 overflow-hidden">
            ...
            <div className="text-center">
              {product.category === 'power-station' ? (
                <div className="relative">
                  <div
                    className="w-28 h-20 rounded-xl border-2 border-yellow/30 bg-dark flex items-center justify-center mx-auto"
                    style={{ boxShadow: '0 0 30px rgba(245,197,24,0.1)' }}
                  >
                    <Battery size={36} className="text-yellow" />
                  </div>
                  <div className="mt-2 flex justify-center gap-2">
                    {[...Array(3)].map((_, i) => (
                      <div key={i} className="w-8 h-2 bg-yellow/20 rounded border border-yellow/30" />
                    ))}
                  </div>
                </div>
              ) : (
                <div
                  className="w-32 h-24 rounded-lg border-2 border-yellow/30 bg-dark flex items-center justify-center mx-auto"
                  style={{ boxShadow: '0 0 30px rgba(245,197,24,0.1)' }}
                >
      ...
      <div className="flex flex-col flex-1 p-5">
        ...
        <Link href={`/products/${product.slug}`} className="group/name">
          <h3
            className="font-bold text-white text-lg leading-tight mb-1 group-hover/name:text-yellow transition-colors"
            style={{ fontFamily: "'Barlow Condensed', sans-serif" }}
          >
            {product.name}
          </h3>
        </Link>
        <p className="text-xs text-yellow font-medium mb-2">{product.tagline}</p>

        {/* Key specs pills */}
        <div className="flex flex-wrap gap-1.5 mb-4">
          {product.capacity && (
            <span className="px-2 py-0.5 bg-white/5 border border-white/10 rounded-full text-xs text-gray-300">
              {product.capacity}
            </span>
          )}
          {product.wattage && (
        ...
        <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between gap-3">
          <div>
            <div className="text-xl font-bold text-white" style={{ fontFamily: "'Barlow Condensed', sans-serif" }}>
              {formatPrice(price, countryCode)}
            </div>
            {originalPrice && (
              <div className="text-xs text-gray-500 line-through">{formatPrice(originalPrice, countryCode)}</div>
            )}
          </div>
          <button
