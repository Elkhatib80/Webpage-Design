...
export default function Logo({ className = '', size = 'md' }: LogoProps) {
  ...
  const pad = size === 'sm' ? 'px-3 py-1.5' : size === 'lg' ? 'px-5 py-2.5' : 'px-4 py-2';

  return (
    <Link href="/" aria-label="smrtQ Solutions home" className={`inline-flex ${className}`}>
      <span className={`flex items-center gap-2 bg-black rounded-xl ${pad}`}>
        {imgState !== 'error' && (
          <Image
